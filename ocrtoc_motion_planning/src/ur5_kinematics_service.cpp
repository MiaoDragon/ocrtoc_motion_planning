/**
* adapted from UR5 kinematics function
**/
#include "ur5_kinematics_service.hpp"
#include <tf/transform_datatypes.h>  // for easier transformation
...
namespace ur5_kinematics
{
    ...
    int inverse(const double* T, double* q_sols, double q6_des) {
        ...
        for(int i=0;i<2;i++) {
            ...
            for(int k=0;k<2;k++) {
                ...
                //if(q_sols[0] < -0.9) continue;
                //if(q_sols[1] > 0 ) continue;
                //if(q_sols[2] < 0) continue;
    ...
    {
        tf::Quaternion q(req.target_pose.orientation.x,req.target_pose.orientation.y,\
                         req.target_pose.orientation.z,req.target_pose.orientation.w);
        tf::Matrix3x3 m(q);
        double q_sols[48], T[12];  // 3x4 matrix, [R|p]
        // read in the transformation
        // position
        //swap(x,y); x=-x; y+=0.24;
        double x = req.target_pose.position.x, y = req.target_pose.position.y, z = req.target_pose.position.z;
        T[3] = x; T[7] = y; T[11] = z;
        //T[3] = req.target_pose.position.x; T[7] = req.target_pose.position.y; T[11] = req.target_pose.position.z;
        for (unsigned i=0; i<3; i++)
        {
            // fill in rotation matrix row by row
            T[i*4+0] = m[i].getX(); T[i*4+1] = m[i].getY(); T[i*4+2] = m[i].getZ();
        }
        int num_sols = inverse(T, q_sols);
        ROS_INFO("number of solutions after IK: %d", num_sols);
        if (num_sols <= 0)
        {
            // no solution found
        ...
        for (unsigned i=0; i<num_sols; i++)
        {
            ...
            solution.name = {"shoulder_pan_joint", "shoulder_lift_joint", "elbow_joint", "wrist_1_joint", "wrist_2_joint", "wrist_3_joint"};
            solution.position = {q_sols[i*6+0],q_sols[i*6+1],q_sols[i*6+2],q_sols[i*6+3],q_sols[i*6+4],q_sols[i*6+5]};
            res.solutions.push_back(solution);
        }
        return true;
    }
}
...
{
  ros::init(argc, argv, "ur5_ik_service");
  ros::NodeHandle n;

  ros::ServiceServer service = n.advertiseService("ur5_ik_service", ur5_kinematics::ur5_ik_service);
  ROS_INFO("Ready to compute IK for UR5.");
  ros::spin();
  return 0;
}